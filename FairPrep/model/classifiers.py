...
class SK_LogisticRegression(Step):
    ...
    def apply(self, df):
        ...
        df[self.pred_target_col] = [x[favorable_class_idx] for x in self.fitted_step.predict_proba(np.array(df.drop(columns=[self.target_col])))]
...
class OPT_DecisionTree(Step):
    ...
    def apply(self, df):
        # dedummy_df = de_dummy_code_df(df)
